...
const kickStreamerUsername = 'endermanch1'; // Kick username
...
async function checkKickLiveStatus() {
  ...
  if (lastKickNotificationDate === today) return; // už dnes jsme poslali notifikaci

  const options = {
    hostname: 'kick.com',
    path: `/api/v2/channels/${kickStreamerUsername}`, // správná interpolace
    method: 'GET',
    headers: {
      'User-Agent': 'Mozilla/5.0', // někdy pomáhá
    },
  };
      ...
      res.on('end', async () => {
        try {
          ...
          if (json && json.livestream && json.livestream.is_live) {
            ...
            if (channel) {
              await channel.send(`📢 **${kickStreamerUsername}** Streamuje živě na https://kick.com/${kickStreamerUsername}`);
              lastKickNotificationDate = today; // nastavíme, že už jsme poslali zprávu dnes
              resolve(true);
              return;
            }
          }
...
setInterval(() => {
  if (client.isReady()) { // zkontrolujeme, že bot je online
    checkKickLiveStatus();
  }
}, 30000);
